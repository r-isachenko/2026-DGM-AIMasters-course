...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{itemize}
		...
		\item $\bv(\bx, \bz, t)$ is a \textbf{conditional vector field}
	\end{itemize}
	\[
		\frac{d\bx}{dt} = \bv(\bx, t) \quad \Rightarrow \quad \frac{d\bx}{dt} = \bv(\bx, \bz, t)
	\]
	\vspace{-0.5cm}
	\begin{block}{Theorem}
...
\begin{frame}{Deriving the Conditional Vector Field}
	...
	\begin{block}{Gaussian Conditional Probability Path}
		...
		\vspace{-0.5cm}
	\end{block}
	Is it possible to derive the expression for $\bv(\bx_t, \bz, t)$?
	\eqpause
	\begin{block}{Statement}
		\vspace{-0.3cm}
		\[
			\bv(\bx_t, \bz, t) =  \bmu_t'(\bz) + \frac{\bsigma_t'(\bz)}{\bsigma_t(\bz)} \odot (\bx_t - \bmu_t(\bz))
		\]
		\vspace{-0.3cm}
	\end{block}
	\eqpause
	...
	\begin{block}{Proof}
		\vspace{-0.2cm}
		\[
			\frac{d\bx_t}{dt} = \bv(\bx_t, \bz, t); \quad \bepsilon = \frac{1}{\bsigma_t(\bz)} \odot (\bx_t - \bmu_t(\bz))
		\]
		\eqpause
		\[
