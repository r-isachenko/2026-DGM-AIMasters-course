...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{align*}
		...
		d\bx &= \left(\bff(\bx, t) - g^2(t) \frac{\partial }{\partial \bx}\log p_t(\bx)\right) dt + g(t) d \bar{\bw} \quad \text{(reverse SDE)}
	\end{align*}
	\vspace{-0.5cm}
	\begin{figure}
...
\begin{frame}{Recap of Previous Lecture}
	...
	\vspace{-0.5cm}
	\[
		\frac{\partial p_t(\bx | \bz)}{\partial t} = - \diver\left(\bff(\bx, \bz, t) p_t(\bx | \bz)\right).
	\]
	\vspace{-0.3cm}
	\begin{itemize}
...
\begin{frame}{Conditional Flow Matching: Continuity Equations}
	...
	\begin{block}{Continuity Equations}
		\vspace{-0.5cm}
		\begin{align*}
			\frac{\partial p_t(\bx)}{\partial t} &= - \diver\left(\bff(\bx, t) \cdot p_t(\bx)\right) \\
			\frac{\partial p_t(\bx | \bz)}{\partial t} &= - \diver\left(\bff(\bx, \bz, t) \cdot p_t(\bx | \bz)\right)
		\end{align*}
		\vspace{-0.5cm}
	\end{block}
	What is the relationship between $p_t(\bx)$ and $\bff(\bx, t)$?
	\eqpause
	\begin{itemize}
		\item $\bff(\bx, t)$ (under regularity conditions) and initial condition $p_0(\bx)$ uniquely determine $p_t(\bx)$.
		\eqpause
		\item But the reverse is not true: any divergence-free vector field $\diver(\tilde{\bff}(\bx, t)p_t(\bx)) = 0$ can generate a valid probability path.
		\eqpause
		\item Moreover, not  every given path $p_t(\bx)$ can actually arise from particles moving under
		some velocity field $\bff(\bx, t)$.
	\end{itemize}
\end{frame}
%=======
\begin{frame}{Conditional Flow Matching: Paths and Vector Fields}
	...
	\begin{block}{Vector fields}
		\vspace{-0.3cm}
		\[
			\bff(\bx, t) = \bbE_{p_t(\bz | \bx)} \bff(\bx, \bz, t) = \int \bff(\bx, \bz, t) \frac{p_t(\bx | \bz) p(\bz)}{p_t(\bx)} d \bz
		\]
		\vspace{-0.5cm}
	\end{block}
	\eqpause
	\begin{itemize}
		\item $\bff(\bx, \bz, t)$ and $p_t(\bx | \bz)$ uniquely determine $\bff(\bx, t)$.
...
\end{frame}
%=======
\begin{frame}{Gaussian Conditional Probability Paths}
	\myfootnotewithlink{https://arxiv.org/abs/2302.00482}{Tong A., et al. Improving and Generalizing Flow-Based Generative Models with Minibatch Optimal Transport, 2023}
	\vspace{-0.3cm}
	\[
		\bbE_{t \sim U[0, 1]} \bbE_{\bz \sim p(\bz)} \bbE_{\bx \sim p_t(\bx | \bz)}\left\| \bff(\bx, \bz, t) - \bff_{\btheta}(\bx, t) \right\|^2 \rightarrow \min_{\btheta}
...
\end{frame}
%=======
\begin{frame}{Deriving the Conditional Vector Field}
	\myfootnotewithlink{https://arxiv.org/abs/2302.00482}{Tong A., et al. Improving and Generalizing Flow-Based Generative Models with Minibatch Optimal Transport, 2023}
