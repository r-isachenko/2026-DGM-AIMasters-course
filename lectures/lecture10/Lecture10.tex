...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\vfill
		\item Converting SDEs to probability flow ODEs enables efficient numerical solvers, reducing the number of inference steps from $100$--$1000$ to $20$--$50$.
		\vfill
		...
		\vfill
		\item For affine-drift SDEs (VE-SDE and VP-SDE), the transition kernel $q(\bx(t) | \bx(0))$ is Gaussian, making the training objective tractable.
		\vfill
		\item Discretizing the reverse SDE yields ancestral sampling, while discretizing the probability flow ODE yields deterministic sampling.
	\end{itemize}
\end{frame}
\end{document}